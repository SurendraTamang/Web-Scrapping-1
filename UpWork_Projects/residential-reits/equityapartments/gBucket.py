...
def send_email(body):
    ...
    msg = EmailMessage()
    msg['Subject'] = 'GOOGLE BUCKET SCRIPT ERROR ALERT: equityApartments'
    msg['From'] = EMAIL_USER
...
def upload_to_bucket(fname):
    ...
    bucket = storage_client.get_bucket("equity_apartments")
...
def main():
    ...
    try:
        ...
        send_email(f'''Hi,\ngBucket.py encountered an error at {datetime.now().strftime("%d/%m/%Y %H:%M:%S")}.\nThe error is: "The equityApartments CSV file is empty. Script terminated without pushing data into Google Bucket".\nPlease see more information here: /home/p.byom26/residentialReits/rrScrapers/equityapartments/gBucket.py\nContact p.byom26@gmail.com for help.\n\nSent From\nGCP Ubuntu VM''')
        os._exit(1)
